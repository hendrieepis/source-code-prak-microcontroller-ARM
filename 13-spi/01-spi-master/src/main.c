...
int main(void)
{
    ...
    spi1_init();
    uart2_init();
    uart_putstr("SPI Master Demo\r\n");

    for (;;)
    {
        resp = spi1_transfer(cnt++);
        uart_putstr("kirim/terima = 0x");
        uart_puthex8(resp);
        uart_putstr("\r\n");
        delay(0xFFFFF);
    }
